...
#define BANNER_RANGE		"sprite/range.png"
...
static void monster_load(){
	monster_img[3] = image_load(MONSTER_LEFT);
	monster_img[1] = image_load(MONSTER_RIGHT);
	monster_img[0] = image_load(MONSTER_UP);
	monster_img[2] = image_load(MONSTER_DOWN);
}
...
SDL_Surface* sprite_get_monster(int dir){
	assert(monster_img[dir]);
	return monster_img[dir];
}
